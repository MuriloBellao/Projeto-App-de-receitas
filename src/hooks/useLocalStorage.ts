...
function useLocalStorage() {
  ...
  const [inProgressRecipes,
    setInProgressRecipes] = useState<InProgressRecipesType>(() => {
  ...
  useEffect(() => {
    ...
    if (doneRecipes.length !== 0) {
      localStorage
        .setItem('doneRecipes', JSON.stringify(doneRecipes));
    }
  ...
  const handleDoneRecipes = (recipe: DoneRecipesType, option: 'add' | 'remove') => {
    if (option === 'add') {
      setDoneRecipes((prevDoneRecipes) => [...prevDoneRecipes, recipe]);
    } else {
      const filteredRecipes = doneRecipes
        .filter((doneRecipe) => doneRecipe.id !== recipe.id);
      setDoneRecipes(filteredRecipes);
    }
  };

  const handleToggleFavorite = async (recipe: FavoriteRecipesType) => {
    const isFavorite = favoriteRecipes.some((favRec) => favRec.id === recipe.id);

    if (isFavorite) {
      await handleFavoriteRecipes(recipe, 'remove');
    }

    // Only remove if the recipe is already in favoriteRecipes
    if (isFavorite) {
      setFavoriteRecipes((prevRecipes) => prevRecipes.filter(
  ...
  FavoriteRecipesType, option: 'add' | 'remove') => {
    if (option === 'add') {
